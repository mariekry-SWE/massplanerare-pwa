// --- GLOBAL FUNKTIONER ---
// Dessa definieras utanför DOMContentLoaded för att säkerställa att de kan anropas både från HTML
// och från andra delar av skriptet (t.ex. i slutet).
// OBS: De använder de globalt tillgängliga variablerna som definieras inuti DOMContentLoaded.
...
const MARGIN = 50; 
let canvas, ctx, workspace; // Dessa kommer att definieras i DOMContentLoaded

function updateCanvasSize() {
    if (!document.getElementById('boothW')) return; // Säkerhetskontroll
...
function parseCSV(text) {
    ...
    if(idxName === -1 || idxW === -1 || idxD === -1) {
         alert(`FEL: Kunde inte hitta kolumnerna Vad, Längd/Bredd, och Djup.\nVi gissade på avgränsare: "${delimiter}"\nRubriker funna: ${headers.join(", ")}`);
         return;
    }
...
// --- Huvudinitiering: Körs när DOM är klar ---
document.addEventListener('DOMContentLoaded', (event) => {
    // Definiera DOM-elementen nu när de är tillgängliga
    canvas = document.getElementById('planCanvas');
    ctx = canvas.getContext('2d');
    workspace = document.getElementById('workspace');

    // Registrera fillyssnare
    ...
    // Registrera andra lyssnare... (drag/pan/zoom)

    // ... (alla drag/pan/zoom lyssnare) ...
    ...
    // Slutligen, initiera canvas när DOM är laddad (denna rad fungerar nu!)
    updateCanvasSize();
});
